...
function punishResultText(icon, label, target, actor, role) {
  return (
    `${icon} ${label}\n` +
    `👤 المستهدف: ${mentionHtml(target)}\n` +
    `👮‍♂️ بواسطة: ${mentionHtml(actor)} (${role})`
  );
}
...
async function handlePunishCommand(env, group, chatId, msg, cmd) {
  ...
  const role = await actorRoleLabel(env, chatId, actor.id);
  ...
  switch (cmd) {
    case "كتم": {
      ...
      await saveGroup(env, group);
      await sendMessage(env, chatId, punishResultText("🔇", "تم كتم العضو", target, actor, role), replyOpts);
      return true;
    }
    case "الغاء كتم": {
      await tg(env, "restrictChatMember", {
      ...
      await saveGroup(env, group);
      await sendMessage(env, chatId, punishResultText("🔊", "تم إلغاء كتم العضو", target, actor, role), replyOpts);
      return true;
    }
    case "حظر": {
      ...
      await saveGroup(env, group);
      await sendMessage(env, chatId, punishResultText("⛔", "تم حظر العضو", target, actor, role), replyOpts);
      return true;
    }
    case "الغاء حظر": {
      ...
      await saveGroup(env, group);
      await sendMessage(env, chatId, punishResultText("✅", "تم إلغاء حظر العضو", target, actor, role), replyOpts);
      return true;
    }
    case "طرد": {
      await tg(env, "banChatMember", { chat_id: chatId, user_id: target.id });
      await tg(env, "unbanChatMember", { chat_id: chatId, user_id: target.id, only_if_banned: true });
      await sendMessage(env, chatId, punishResultText("👢", "تم طرد العضو", target, actor, role), replyOpts);
      return true;
    }
    case "تقييد": {
      await tg(env, "restrictChatMember", {
      ...
      await saveGroup(env, group);
      await sendMessage(env, chatId, punishResultText("🚧", "تم تقييد العضو", target, actor, role), replyOpts);
      return true;
    }
    case "الغاء تقييد": {
      await tg(env, "restrictChatMember", {
      ...
      await saveGroup(env, group);
      await sendMessage(env, chatId, punishResultText("✅", "تم إلغاء تقييد العضو", target, actor, role), replyOpts);
      return true;
    }
    default:
      return false;
...
async function handleEditedGroupMessage(env, editedMsg) {
  ...
  const senderIsAdmin = await isGroupAdmin(env, chatId, userId);
